import pygame
buttonclicktime = 100
widgets = [] # a list with the position of all widgets and their class
textboxActive = False

def CheckForClick(mouseposx : int, mouseposy : int) -> None:
...
class Button :
    def __init__(self, position : tuple, buttonSize : tuple, color : tuple, canvas : object, buttonFunction, text : str = "") -> None:
        '''position in (x,y) with topleft anchor\n
        buttonSize in (x,y)\n
        color in RGB\n
        canvas in drawmap object\n
        buttonFunction in functionname without ()'''
        self.buttonRect = pygame.Rect(position[0], position[1], buttonSize[0],buttonSize[1])
        self.size = (position[0], position[1], buttonSize[0],buttonSize[1])
        self.color = color
        self.canvas = canvas
        self.function = buttonFunction
        self.text = text
        self.canvas.DrawRect(self.size, self.color)
        self.Textoperation()
        widgets.append((self.buttonRect, self))
        
    def Clicked(self, state : bool) :
    ...
    def ClickColorChange(self) -> None:
        ...
        self.canvas.DrawRect(self.size, (max(self.color[0]- 50, 0), max(self.color[1]- 50, 0), max(self.color[2]- 50, 0)))
...
class InputBox :
    def __init__(self, position : tuple, size : tuple, canvas : object, fontsize : int = 18, text : str = "") -> None:
        ...
        self.textBoxRect = pygame.Rect(position[0], position[1], size[0], size[1])
        self.textposition = self.textBoxRect[0]+20, self.textBoxRect[1]+ self.textBoxRect[3]//2
